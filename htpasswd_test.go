...
func testSystem(t *testing.T, name string, contents string) {
	...
	for _, u := range testUsers {
		...
		if good := htp.Match(u.username, u.password); !good {
			t.Errorf("%s user %s, password %s failed to authenticate: %d", name, u.username, u.password, good)
		}

		notPass := u.password + "not"
		if bad := htp.Match(u.username, notPass); bad {
			t.Errorf("%s user %s, password %s erroneously authenticated: %d", name, u.username, notPass, bad)
		}
	}
